...
在java中，所有实例域、静态域和数组元素存储在堆内存 (heap)中，堆内存在线程之间共享（本文使用“共享变量”这个术语代指实例域，静态域和数组元素）。
...
3. 如果不存在数据依赖性，处理器可以改变语句对应机器指令的执行顺序。
...
[JVM内存的那些事](http://www.importnew.com/23629.html) un
[]()
[]()
[]()
