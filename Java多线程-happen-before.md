# Java多线程 happen-before



JDK 5 开始，JMM就使用happens-before的概念来阐述多线程之间的内存可见性。

happens-before原则是判断数据是否存在竞争、线程是否安全的主要依据，依靠这个原则，我们解决在并发环境下两操作之间是否可能存在冲突的所有问题。下面我们就一个简单的例子稍微了解下happens-before ；
...
约定线程A执行write()，线程B执行read()，
且线程A优先于线程B执行，那么线程B获得结果是什么？；
...
![](./pics/happen-before-JMM-relations.png)


##参考资料


[Java内存模型之happens-before](http://www.cnblogs.com/chenssy/p/6393321.html?utm_source=tuicool&utm_medium=referral)
[深入理解Java虚拟机]()
[【死磕Java并发】—–Java内存模型之happens-before](http://cmsblogs.com/?p=2102)
[深入Java内存模型—happen-before规则及其对DCL的分析（含代码）](http://www.importnew.com/20763.html) un
[happens-before俗解](http://ifeve.com/easy-happens-before/)   un
